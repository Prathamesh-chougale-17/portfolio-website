    ...
    return (
        <section className="section-padding px-4 bg-background">
            <div className="text-center mb-16">
                ...
                <h2 className="text-3xl md:text-4xl font-bold text-foreground">Media Mentions</h2>
                ...
                <div className="rounded-xl bg-card/50 dark:bg-white/5 backdrop-blur-sm p-8 md:p-12 relative shadow-md border border-border">
                    ...
                    >
                                ...
                                <CarouselItem key={index} className="flex items-center justify-center">
                                    <div className="text-center">
                                        ...
                                        <div className="space-y-1">
                                            <h4 className="font-semibold text-lg text-foreground">{mention.title}</h4>
                                            <p className="text-muted-foreground">{mention.publication}, {mention.year}</p>
                                        </div>
                                    </div>
                                </CarouselItem>
                            ))}
                        ...
                        <div className="flex justify-center items-center space-x-4">
                            <CarouselPrevious className="border border-border hover:bg-accent absolute transform-none text-foreground" variant="ghost" />
                            <div className="flex items-center space-x-2">
                                {Array.from({ length: totalSlides }).map((_, index) => (
                                    <button key={index} className={cn("w-2 h-2 rounded-full transition-all", index === current ? "bg-primary w-6" : "bg-muted hover:bg-muted-foreground/50")} onClick={() => api?.scrollTo(index)} aria-label={`Go to slide ${index + 1}`} />
                                ))}
                            </div>
                            <CarouselNext className="border border-border hover:bg-accent absolute transform-none text-foreground" variant="ghost" />
                        </div>
                    </Carousel>
                </div>
            </div>
